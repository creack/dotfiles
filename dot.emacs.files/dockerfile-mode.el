...
(defvar docker-host nil)

(setq docker-host "172.17.8.101:4243")
...
(defun dockerfile-build-buffer (image-name)
  "Build an image based upon the buffer with tag IMAGE-NAME."
  (interactive
  ...
  (if (stringp image-name)
      (shell-command
       (concat (if dockerfile-use-sudo "sudo " "") "docker -H " docker-host " build -t " image-name " " (file-name-directory (buffer-file-name)) "&")
       "*docker-build-output*")
