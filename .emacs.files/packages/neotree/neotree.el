...
(defun neo-buffer--insert-file-entry (node depth &optional cur-file-name)
  (let ((node-short-name (neo-path--file-short-name node))
...
(defun neo-buffer--insert-tree (path depth &optional cur-file-name)
  (if (eq depth 1)
  ...
  (let* ((contents (neo-buffer--get-nodes path))
    ...
    (dolist (leaf leafs)
      (neo-buffer--insert-file-entry leaf depth cur-file-name))))

(defun neo-buffer--refresh (save-pos-p &optional non-neotree-buffer &optional cur-file-name)
...
If SAVE-POS-P is non-nil, it will be auto save current line number."
  (let ((start-node neo-buffer--start-node))
    ...
    (neo-buffer--with-editing-buffer
     ...
     (neo-buffer--insert-tree start-node 1 cur-file-name))
    ;; restore context
  ...
  (if (eq (current-buffer) (neo-global--get-buffer))
    ...
    (save-excursion
      (let ((cw (selected-window)))  ;; save current window
        (neo-buffer--refresh t t buffer-file-name)
